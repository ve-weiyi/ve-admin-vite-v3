...
/** 删除相册 */
export function deletePhotoAlbumApi(id: string): Promise<IApiResponseData<any>> {
...
/** 查询相册 */
export function findPhotoAlbumApi(id: string): Promise<IApiResponseData<PhotoAlbum>> {
	return http.request<IApiResponseData<PhotoAlbum>>({
		url: `/api/v1//photo_album/${id}`,
		method: "get",
	})
...
/** 分页获取相册列表 */
export function findPhotoAlbumListApi(page: PageQuery): Promise<IApiResponseData<PageResult<PageResult<PhotoAlbum>>>> {
	return http.request<IApiResponseData<PageResult<PageResult<PhotoAlbum>>>>({
